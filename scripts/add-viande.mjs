...
const newProduct = 'Viande hachée';

const style = { header: 'F44336', headerFont: 'FFFFFF', light: 'FFEBEE', medium: 'FFCDD2' };
const thinBorder = {
...
async function addToFile(filePath, sheetName) {
  ...
  console.log(`✅ ${path.basename(filePath)} — added ${newProduct} (${lastRow - 2} produits total)`);
}

await addToFile(path.join(dir, 'Viande.xlsx'), 'Viande');
await addToFile(path.join(dir, 'Inventaire_Complet.xlsx'), 'Viande');
