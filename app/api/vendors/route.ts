...
// part of the purchasing/achat flow).
export async function GET(request: NextRequest) {
    const denied = await enforceAny(['finance.read', 'inventory.read']); if (denied) return denied;
  try {
...
export async function POST(request: NextRequest) {
    // Achat flow (recording a purchase) creates vendors/transactions, so allow
