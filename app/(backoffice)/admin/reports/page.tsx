...
// counts_as_cash = false → "Déduit": amount SUBTRACTED from BOTH Total Espèces and Espèce Caisse.
...
const cashSheetTotals = (sheet: CashSheet) => {
  ...
  // R1 — extra encaissements. "Espèce" (toggle ON) ADDS to Total Espèces only; the drawer
  // (Espèce Caisse) stays untouched. "Déduit" (toggle OFF) SUBTRACTS from BOTH Total Espèces
  // AND Espèce Caisse.
  const especeSources = (sheet.payment_sources || [])
    .filter(s => s.counts_as_cash)
    .reduce((s, src) => s + parseCashNumber(src.amount), 0);
  const deduitSources = (sheet.payment_sources || [])
    .filter(s => !s.counts_as_cash)
    .reduce((s, src) => s + parseCashNumber(src.amount), 0);
  ...
  // Espèce caisse (cash in the drawer) = CA − CB − les deux Glovo − les "Déduit".
  const caisseEspeces = Math.max(0, totalCA - totalCB - glovoEspece - glovoOnline - deduitSources);
  // Total espèces = CA − CB − Glovo online − les "Déduit", PLUS les "Espèce".
...
export default function ReportsPage() {
  ...
  const exportSuiviExcel = async () => {
    ...
    hRow2.values = [
      '',
      'Carte', 'Espèces', 'Virement', 'Glovo TTC Espèce', 'Glovo TTC Online', 'Total',
    ...
    for (const e of sorted) {
      ...
      const row = ws.addRow([
        ...
        Number(e.revenue_card), Number(e.revenue_cash), Number(e.revenue_transfer), Number(e.glovo_ttc_espece || 0), Number(e.glovo_ttc_online || 0), revTotal,
      ...
      row.getCell(7).font = { bold: true, color: { argb: '606338' } };
      row.getCell(11).font = { bold: true };
      row.getCell(14).font = { bold: true };
      row.getCell(15).font = { bold: true, color: { argb: Number(e.solde_theorique) >= 0 ? '16A34A' : 'DC2626' } };
      row.getCell(16).font = { bold: true, color: { argb: '0070C0' } };
    }

    // Total row
    const totalRow = ws.addRow([
      'TOTAL',
      totRevCard, totRevCash, totRevTransfer, totGlovoTtcEspece, totGlovoTtcOnline, totRevCard + totRevCash + totRevTransfer + totGlovoTtcEspece + totGlovoTtcOnline,
    ...
    ws.columns = [
      { width: 12 },
      { width: 12 }, { width: 12 }, { width: 12 }, { width: 14 }, { width: 14 }, { width: 14 },
                  ...
                  <div className="bg-card border border-border rounded-xl p-3">
                    ...
                    {currentCashSheetTotals.especeSources > 0 && (
                      <div className="mt-2 pt-2 border-t border-border text-[11px] text-muted-foreground flex justify-between">
                        ...
                        <span className="font-medium text-[#606338]">+{fmtMAD(currentCashSheetTotals.especeSources)}</span>
                      </div>
                    )}
                    {currentCashSheetTotals.deduitSources > 0 && (
                      <div className="mt-1 text-[11px] text-muted-foreground flex justify-between">
                        <span>Déduit des espèces (total + caisse) :</span>
                        <span className="font-medium text-red-600">−{fmtMAD(currentCashSheetTotals.deduitSources)}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <table className="w-full text-xs whitespace-nowrap">
                    <thead>
                      ...
                      <tr className="bg-secondary/70 text-[10px] text-muted-foreground border-t border-border">
                        ...
                        <th className="px-2 py-2 text-right font-medium">Glovo TTC Espèce</th>
                        <th className="px-2 py-2 text-right font-medium">Glovo TTC Online</th>
                    ...
                    <tbody className="text-foreground">
                      {monthEntries.map(e => (
                        ...
                        >
                          ...
                          <td className="px-2 py-2 text-right tabular-nums text-purple-600">{fmtMADShort(Number(e.glovo_ttc_espece || 0))}</td>
                          <td className="px-2 py-2 text-right tabular-nums text-purple-600">{fmtMADShort(Number(e.glovo_ttc_online || 0))}</td>
